...
Although technically quite straight forward, in practice much harder. Formatting of the fields in, for instance 
...
I credit [Bealdung](https://www.baeldung.com/). Everytime I needed to explore something
new (at least new to me), most of the time I found myself ending up there for great answers, explanations, and samples.

Recently I upgraded to Spring boot 4 and Spring 7. Except for mandatory changes to POM and code, I introduced 
no new features. On my wishlist:
- More functional style programming, e.g. using vavr library
- Add open telemetry
- Turn into multi-module maven POM with the core separated from the adapters
- use batch updates when storing keywords, book authors, etc.

### Domain model
The domain model started simple: a BOOK has an identity (an ISBN, of course), some attributes, a set of KEYWORDS, 
...
said, the identity had different formats, it was certainly not always and ISBN, and even when a UUID was used 
as identity, it proved not to be unique (so much for identity)!
...
Parsing the data in a way that matches the domain model took some time and inspiration, and for sure cannot always be 
done right ;-)
...
The persistence implementation is unit-tested using the ```@JdbcTest``` annotation from Spring, and a ```schema.sql```
...
Tika, but the impact was limited. Tika provides a```parseToString(InputStream stream, Metadata metadata, int maxLength)``` 
...
The service implementations have the `@Transactional` annotation, which means they form the boundary of any 
transactional persistent change.

### Web adapters for REST endpoints
And now finally, the step to link everything to the World Wide Web. I started with the ```AuthorController```, as it's 
...
To prevent the domain model being exposed directly through the web (also because it created quite ugly JSON), POJO's 
...
API versioning has been implemented using path-segment versioning. All API endpoints urls start with 
```/api/v{version}``` (except for graphql, error, and actuator endpoints).
...
overdone indeed). The token only contains the user authorities (stored in a claim called 'authorities') as 
...
method calls (including input parameter values), and on method return.

Logging is configured in the `application.yml` file (not the `logback.xml` file). 

### SSL
Enabling SSL was quite simple, and the process is described at 
...
as a parameter. Somehow Spring didn't take a ```HttpServletRequest``` as first parameter to the handler. When the type of 
...
### Set<?> instead of List<?>
When reading on JPA and Hibernate, I read about using ```Set``` instead of ```List```, as a Set is safer, because 
...
for SSL connections. The ```SSLContext``` needs access to the certificate (trust material). To prevent additional 
issues, the ```SSLConnectionSocketFactory``` must not check the host name of the certificate (use 
...
A possible next step is to replace the use of the RestTemplate with something like 
